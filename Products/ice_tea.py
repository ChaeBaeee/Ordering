...
ASSETS_PATH = OUTPUT_PATH / Path(r"C:\Users\pilim\Desktop\build\assets\frame18")
...
def create_ice_tea_content(window):
    items = load_items()  # Refresh items from database
    if "Classic Iced Tea" not in items:
        print("Error: 'Classic Iced Tea' not found in items")
        return  # Exit the function if the key is not found
    ice_tea_price = items["Classic Iced Tea"]["price"]  # Ensure this key matches the one in the database.json
    ice_tea_stock = items["Classic Iced Tea"]["stock"]  # Ensure this key matches the one in the database.json
    quantity = 1  # Initialize quantity variable
    print("Creating Classic Iced Tea content...")
    for widget in window.winfo_children():
        widget.destroy()  # Clear existing content

    window.geometry("507x782")
    window.configure(bg = "#BF8040")
    ...
    canvas.create_text(
        ...
        417.8765869140625,
        anchor="nw",
        text=f"Php {ice_tea_price}",  # Use the dynamic price
        fill="#FFFFFF",
        font=("Abril Fatface", 20 * -1)
    )
