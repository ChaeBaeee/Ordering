...
ASSETS_PATH = OUTPUT_PATH / Path(r"C:\Users\pilim\Desktop\build\assets\frame20")
...
def create_pepsi_content(window):
    items = load_items()  # Refresh items from database
    pepsi_price = items["Pepsi"]["price"]  # Get the price of Pepsi
    pepsi_stock = items["Pepsi"]["stock"]  # Get the stock of Pepsi
    quantity = 1  # Initialize quantity variable
    print("Creating Pepsi content...")
    for widget in window.winfo_children():
        widget.destroy()  # Clear existing content

    window.geometry("507x782")
    window.configure(bg = "#0033A0")
    ...
    button_1 = Button(
        ...
        borderwidth=0,
        highlightthickness=0,
        command=lambda: add_to_cart("Pepsi", quantity),  # Remove popup message
        relief="flat",
        state="disabled" if pepsi_stock == 0 else "normal",  # Disable if stock is 0
        text="Unavailable" if pepsi_stock == 0 else ""  # Add "Unavailable" text if stock is 0
    )
    button_1.place(
        x=344.0,
        y=712.0,
    ...
    canvas.create_text(
        ...
        417.8765869140625,
        anchor="nw",
        text=f"Php {pepsi_price}",  # Use the dynamic price
        fill="#FFFFFF",
        font=("Abril Fatface", 20 * -1)
    )
